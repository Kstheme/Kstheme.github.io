...
class SelfAttentionV3(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        # if attention_mask is not None, we need to assign an extremely small value to the masked tokens 鈥斺€?this way, their value will be 0 after applying Softmax.
...
1. **Phase 1: Engineering efficiency optimization (V1 -> V2)**
    ...
    - **Value**: Transition from a "teaching-level redundant implementation" to "engineering-efficient implementation" 鈥斺€?no performance loss, only efficiency improvement.
2. **Phase 2: Functionlity completeness optimization (V2 -> V3)**
    ...
    - **Problem Solved**: Regularization 鈥斺€?prevent the model from over-relying on a few key positions and alleviate overfitting.
