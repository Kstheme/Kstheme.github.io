 ...
 */
export const enCollections = defineCollections([enBlog, enDemoDoc]);
...
const zhDemoDoc = defineCollection({
  ...
  // `dir` 所指向的目录中的所有 markdown 文件，其 permalink 需要以 `linkPrefix` 配置作为前缀
  // 如果 前缀不一致，则无法生成侧边栏。
  // 所以请确保  markdown 文件的 permalink 都以 `/zh/` + `linkPrefix` 开头
  linkPrefix: "/demo",
  // 文档标题，它将用于在页面的面包屑导航中显示
  title: "Demo",
  // 手动配置侧边栏结构
 ...
 */
export const zhCollections = defineCollections([zhBlog, zhDemoDoc]);
